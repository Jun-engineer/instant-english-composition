    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/30 px-4 py-8 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <header className="mb-4 text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900">お気に入り単語リスト</h2>
          <p className="mt-1 text-sm text-slate-500">復習したい単語をいつでも確認できます。</p>
        </header>
        {orderedFavorites.length === 0 ? (
          ...
          <ul className="max-h-[60vh] space-y-4 overflow-y-auto pr-1">
            {orderedFavorites.map((favorite) => (
              <li key={`${favorite.word}-${favorite.savedAt}`} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
